...
struct Account {
    ...
    // The scope of child contracts included in the agreement. Immutable once added — there is
    // no in-place setter. Updating requires `removeAccounts` + re-add, which is blocked during
    // the commitment window. Choose carefully at add time.
