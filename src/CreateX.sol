...
/// @dev This has been modified from the original CreateX implementation to add virtual keywords, and update the
/// solidity version
...
contract CreateX {
     ...
     */
    function deployCreate(bytes memory initCode) public payable virtual returns (address newContract) {
     ...
     * @return newContract The 20-byte address where the contract was deployed.
     */
    function deployCreate2(bytes32 salt, bytes memory initCode) public payable virtual returns (address newContract) {
     ...
     * @return newContract The 20-byte address where the contract was deployed.
     */
    function deployCreate2(bytes memory initCode) public payable virtual returns (address newContract) {
     ...
     */
    function deployCreate3(bytes32 salt, bytes memory initCode) public payable virtual returns (address newContract) {
        bytes32 guardedSalt = _guard({ salt: salt });
     ...
     * @return newContract The 20-byte address where the contract was deployed.
     */
    function deployCreate3(bytes memory initCode) public payable virtual returns (address newContract) {
        // Note that the safeguarding function `_guard` is called as part of the overloaded function
        // `deployCreate3`.
